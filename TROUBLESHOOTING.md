...
## 15. Secret 恢复发生冲突并回显编码字段

**状态：已实际验证；凭证轮换待完成**

### 现象

错误凭证演练后，直接对旧 Secret 备份执行 `kubectl apply`，API Server 因
`resourceVersion` 已变化而拒绝更新。客户端的冲突诊断同时打印了待提交 patch，其中包含
真实 AK/SK 的 base64 编码值。

### 根因

Secret 备份保留了修改前的 `resourceVersion`。`kubectl apply` 在三方合并冲突时可能输出
完整 patch；base64 只是编码，不是脱敏或加密，能够直接还原原值。

### 修复

1. 不复制、不再次打印冲突输出；
2. 从权限为 `0600` 的备份读取原字段，直接覆盖当前 Secret；
3. 重启 Operator，并等待云侧 VPC/SubENI 同步成功；
4. 安全删除临时备份；
5. 在云 IAM 侧轮换已回显的 AK/SK，再更新 Kubernetes Secret；
6. 后续恢复按 `INSTALL-DEPLOY.md` 先刷新备份中的 `resourceVersion`，使用 `replace`，且不
   允许错误输出进入共享日志。

### 验证

原 Secret 已恢复，Operator 再次成功同步云资源，副本和节点全部 Ready，临时备份已删除。
集群恢复不能证明旧凭证仍然安全，因此凭证轮换完成前，本项不能视为安全闭环。

## 16. Secret 键名检查命令被 shell 提前展开

**状态：已实际验证**

### 现象

使用双引号包裹 jsonpath，并在其中使用 `$k`、`$v` 时，远端 shell 先展开变量，导致
jsonpath 解析错误。

### 根因

模板变量和 shell 变量使用相同的 `$` 语法；引用层级错误使模板在传给 `kubectl` 前已经
被破坏。

### 修复

只需确认固定键时直接逐项查询键是否存在，不遍历值；需要遍历时，把整个模板放在单引号中，
并确保外层远程命令不会再次解释 `$`。任何检查都不得输出 `.data` 的值。

### 验证

错误命令未读取或输出 Secret 值；后续检查改为固定的 `secretKeyRef` 名称和键名核对。
...
初始化阶段把 Deployment 缩容到零后立即重新扩容，首轮查询同时读到了正在终止的旧 Pod
和新 Pod，导致数量高于目标值。脚本用 `条件一 && 条件二` 断言，在 `set -e` 下没有按预期
退出，首轮结果无效。

### 根因

脚本没有在第一次扩容前等待旧 Pod 完全删除；位于 `&&` 条件列表中的失败命令也属于 shell
的 `errexit` 例外场景。

### 处理

按测试要求停止该场景，终止循环并删除测试命名空间和临时结果。以后若重新执行，缩容后先
显式等待 Pod 数量为零，并使用独立的 `if ...; then ...; else exit 1; fi` 断言。

### 验证

远端没有残留测试进程或命名空间，相关 Pod IP 在 `status.ipam.used` 中的残留数量为零；
节点、Agent、Operator 和 CoreDNS 均保持 Ready。该轮数据没有写入性能结论。
