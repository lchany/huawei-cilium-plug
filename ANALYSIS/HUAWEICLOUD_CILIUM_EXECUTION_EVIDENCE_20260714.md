...
# Live GSO/GRO/checksum offload matrix audit101

- The real `eth0` feature baseline was first checked on all five hosts: RX checksum is `on [fixed]`; RX/TX VLAN offload is `off [fixed]`; TX checksum, scatter-gather, TSO, GSO, and GRO are enabled and mutable. Because the fixed features cannot be switched by this ECS NIC, the executable matrix varied all mutable segmentation/checksum features on both the cross-node traffic source (`node0001`) and destination (`node0002`).
- Five asserted states were exercised: baseline; GRO off; GSO off; TX checksum off; and GRO/GSO/TSO/scatter-gather/TX-checksum all off. Each state passed a fresh full eight-Pod mesh at `56/56`, three 100-packet same-node ICMP runs, three 100-packet cross-node ICMP runs, three same-node 8MiB TCP transfers, and three cross-node 8MiB TCP transfers. Totals were five state assertions, five in-state meshes, 3000 ICMP packets with zero loss, and 30 successful 8MiB TCP transfers. The runner restored the original mutable feature set, asserted every value, and passed a sixth `56/56` mesh.
- The runner uses the shared destructive-suite lock for its entire lifetime and the matrix helper supports an explicitly inherited lock, preventing another customer/destructive suite from overlapping feature changes. Bash syntax, shellcheck, whitespace review, and an inherited-lock `56/56` smoke test passed. Runner SHA256 is `fa3874a7e65f138c64ef77f5e4f3e16528bf2f489c9ad3a2ef2934c73fb54b1d`. Evidence: `/tmp/audit101-offload-matrix.out`, `/tmp/audit101-restored-features.out`, `/tmp/audit101-live-health.out`, and `/tmp/audit101-inherited-lock-smoke.out`.
- Final health shows all five nodes Ready, five Agents Ready with zero restarts, and audit90 Operator `1/1`. No product or BPF source changed, so audit95 Agents remain authoritative and no rebuild was required. This closes VLAN-15. The ledger is now 402 Pass, 39 Pending, 10 user-approved Skips, and 0 Fail.

# Exact HuaweiCloud BPF map capacity audit102

- A privileged Go test loads the exact map definitions from the compiled `huaweicloud_test.o`, asserts both `cilium_hwc_srcip4` and `cilium_hwc_vlan_mac` retain the production `max_entries=65536`, and creates isolated unpinned instances with the same key/value schemas. Each map is filled from empty through all 65536 slots. A distinct 65537th key must be rejected; after deleting one existing key, that previously rejected key must insert successfully. This proves near-full operation, hard-cap enforcement, and immediate slot reuse without touching live pinned maps.
- An excluded first attempt preserved `LIBBPF_PIN_BY_NAME` on the isolated MapSpec and therefore failed before capacity execution because no pin path was supplied. The test was corrected by copying the production spec and disabling only pinning for the ephemeral instance; schemas, type, and capacity remain exact. The corrected focused test passed verbosely and 20 repetitions, exercising more than 2.6 million successful inserts across both maps. All ten BPF objects plus the capacity test passed together once.
- A clean 15-patch replay repeated the capacity test 20 times and ran the complete ten-object privileged BPF package once. Generated `.o`/`.ll`/`.s` outputs and the superseded audit100 replay were removed. Test source SHA256 is `e066900dd5731e364699030717678aceed811b161eb3cc9573aa8212c6c35b34`. Authoritative source `8d1ac5a642408c95fcfa822e7438b44f22ff8558`, consolidated patch-generation commit `c032782492cdb30e17106077c90fcf41750b7b76`, and clean 15-patch replay `636d531ca32fb1b631762fe942b3c2bd0007173d` share tree `b16f850a6a8402d781897484802af06726326137`. Patch 0015 SHA256 is `92f78ed9db78c00df9ae6c8c3956c6989f2f23d09f18ef9ff1d114cd9410805c`. Evidence: `/tmp/audit102-bpf-compile-final.out`, `/tmp/audit102-map-capacity-final2.out`, `/tmp/audit102-map-capacity-count20.out`, `/tmp/audit102-bpf-all-compile.out`, `/tmp/audit102-bpf-all-run.out`, `/tmp/audit102-replay-bpf-compile.out`, `/tmp/audit102-replay-capacity-count20.out`, `/tmp/audit102-replay-bpf-all-compile.out`, and `/tmp/audit102-replay-bpf-all-run.out`.
- Product/BPF runtime code did not change, so no Agent rebuild was needed. Five nodes and five zero-restart Agents remained Ready, audit90 Operator remained `1/1`, and post-test mesh passed `56/56`. Evidence: `/tmp/audit102-post-mesh.out` and `/tmp/audit102-live-health.out`. This closes BPF-08. The ledger is now 403 Pass, 38 Pending, 10 user-approved Skips, and 0 Fail.
