# HuaweiCloud Cilium v1.12.19 五节点实机测试场景全集
...
- 推荐拓扑：1 个 control-plane、4 个 worker，覆盖至少 2 个 AZ；
- 详细命令与核心用例见
  `ANALYSIS/HUAWEICLOUD_CILIUM_5_NODE_LIVE_TEST_CASES.md`；
- 源码边界缺口、风险假设和补齐结果见
  `ANALYSIS/HUAWEICLOUD_CILIUM_BOUNDARY_COVERAGE_REVIEW.md`；
- 客户原始配置适配和 25 个强制验收用例见
  `ANALYSIS/HUAWEICLOUD_CILIUM_CUSTOMER_ACCEPTANCE_CASES.md`；
- 本文只记录场景、预期和证据要求，不记录真实 AK/SK、项目 ID、VPC ID、子网 ID、
  安全组 ID、公网 IP 或其他敏感资源标识。

场景类型：

- `R`：真实云环境直接执行；
- `C`：受控故障或破坏性场景，只能在测试资源和变更窗口执行；
- `S`：源码、构建、模拟 API 或离线检查，不能仅靠实机黑盒完成；
- `O`：可选场景，取决于客户是否启用对应能力。

优先级：`P0` 为发布阻断，`P1` 为发布前应完成，`P2` 为长期质量或兼容性补充。

## 2. 五节点覆盖模型

| 节点 | AZ | 主要用途 |
| --- | --- | --- |
| `cp-01` | AZ-A | Kubernetes 控制面、Helm、Operator、证据汇总 |
| `worker-a1` | AZ-A | 同节点流量、Pod 子网 A、性能基线 |
| `worker-a2` | AZ-A | 同 AZ 跨节点、Pod 子网 A、滚动升级 |
| `worker-b1` | AZ-B | 跨 AZ、Pod 子网 B、Service/Policy |
| `worker-b2` | AZ-B | 双网关路由、故障注入、回收和容量边界 |

至少准备：

1. 同一 VPC 内两个 AZ 的专用 Pod 子网；
2. 同一 AZ 内两个网关不同、可供同一节点选择的 Pod 子网，用于验证 `0007`；
3. 一个显式 ID 子网、一个标签选择子网、一个无匹配/容量耗尽子网；
4. 显式安全组、标签安全组、继承 trunk 安全组三条路径；
5. 只属于测试资源的标签、临时凭据和证据目录。
...
执行步骤：
实际结果：
状态：Pass / Fail / Block / NotRun
缺陷编号：
日志、抓包、CiliumNode、云资源快照、性能数据路径：
清理与恢复结果：
```

任何证据落盘前都必须脱敏。外部 Secret 只能核对对象名和键名，不得导出 `.data` 值。

## 4. Patch 到实机场景追踪

| Patch | 功能 | 重点场景 |
| --- | --- | --- |
| `0001` | HuaweiCloud API、metadata、IPAM、SubENI 生命周期、Helm/CRD | API、IPAM、配额、回收、SG、标签、metadata |
| `0002` | CNI、Daemon、BPF map、VLAN 数据面 | CNI 配置、map 生命周期、Pod 双向流量 |
| `0003` | 依赖、生成物、自动化测试、netprobe | patch 重放、生成物、组件测试、实机探针 |
| `0004` | Operator 默认命令 | 镜像 `Config.Cmd`、通用二进制启动 |
| `0005` | 保留 variant Operator 二进制 | Helm 显式命令、variant 二进制启动 |
| `0006` | 按标签选子网及配置合并 | 标签矩阵、ID 优先、`.conf/.conflist`、字段合并 |
| `0007` | 每个 SubENI 独立策略路由表 | 双网关、VLAN 表号、遗留规则清理、重启恢复 |
| `0008` | 线内/metadata VLAN 入方向处理 | 802.1Q、802.1ad、双表示、通用 VLAN 过滤回归 |
| `0009` | 外部 Secret 注入 Operator 凭据 | 缺失/错误 Secret、轮换、Helm release 无凭据、卸载保留 |
| `0010` | 同步子网真实可用地址数 | 容量 0/不足/相等、分页、错误响应、候选子网排序 |
...
### 5.2 ECS、Kubernetes、metadata 与 trunk 前置条件
...
### 5.5 HuaweiCloud API 正常、异常与最终一致性
...
### 5.8 BPF map 与 VLAN 数据面（0002、0008）
...
### 5.9 Pod、Node、Service、DNS 与出口网络
...
| SVC-08 | P1/O | ExternalIP | 客户环境允许时验证正反向路径 |
...
| STAB-01 | P1/R | 24 小时持续探测 | 成功率达标，无资源持续增长 |
| STAB-02 | P1/R | 24 小时 Pod churn | 每 10 分钟扩缩容，状态始终收敛 |
| STAB-03 | P2/R | 72 小时发布候选 | 无 P0/P1 缺陷、泄漏或性能漂移 |
...
### 6.1 metadata、region 与 trunk 选择边界

| ID | P/类型 | 边界输入或状态 | 必须断言 |
| --- | --- | --- | --- |
...
| BAPI-07 | P0/S | 批创建返回 `nil` 数组 | 不得把“创建 0 个”当作满足本轮分配 |
...
| BIPAM-06 | P0/S | BatchCreate 成功但返回空列表 | 本轮不应报告成功满足 allocation |
...
| BRACE-12 | P1/S | 24 小时并发 churn + resync | 使用 race/一致性检查确认无锁问题和资源漂移 |

## 7. 发布门禁

1. 所有 P0 场景必须 Pass；实机无法构造的分支必须有等价受控测试通过和明确的环境限制说明，
   不得让 P0 长期停留在 Block；任何 P0 Fail 均阻断发布。
2. `0007` 双网关隔离、`0008` VLAN 表示矩阵、`0009` 外部 Secret 契约必须有实机证据。
3. 五节点同节点、同 AZ、跨 AZ、Service、DNS、NetworkPolicy、VPC 内网和公网路径通过。
4. Agent、Operator、worker 重启以及升级/回滚通过。
5. 至少完成 24 小时稳定性，且无 SubENI、port、IP、route、BPF map 或 Kubernetes 资源泄漏。
6. 凭据不进入 values、命令行、Helm release、日志、事件或测试报告。
7. 所有镜像 digest、patch commit、脱敏配置、云资源快照和证据路径可追溯。

## 8. 执行分组建议

| 阶段 | 场景组 | 建议时长 |
| --- | --- | --- |
| D0 | BASE、PRE、SEC 离线预检 | 0.5～1 天 |
| D1 | ECS/Kubernetes/metadata/trunk、安装 | 1 天 |
| D2 | API、IPAM、子网、SG、配额 | 1～2 天 |
| D3 | ROUTE、BPF、VLAN、基础网络 | 1～2 天 |
| D4 | Service、DNS、NetworkPolicy | 1 天 |
| D5 | 故障、升级、回滚、清理演练 | 1～2 天 |
| D6 | 性能测试并启动 24 小时稳定性 | 1 天 |
| D7 | 稳定性收尾、缺陷复测、报告 | 1 天 |

高风险 `C` 场景一次只操作一个 worker；执行前后都要保存 CiliumNode、Pod、SubENI、
route、BPF map、Helm revision 和云资源快照。

## 9. 执行前待填参数

- 五台 ECS 的节点名、AZ、规格、OS、内核、CPU 架构和 trunk 网卡；
- 采用 1+4 还是 3+2 拓扑；
- VPC CIDR、Pod 子网、双网关子网、标签、安全组和 SubENI 配额；
- Agent、Operator、netprobe 镜像地址及 digest；
- 测试账号最小权限和 `existingSecret` 名称；
- VPC 内网、公网、DNS、LoadBalancer 测试目标；
- Pod Ready、恢复时间、成功率、吞吐和 P99 RTT 门限；
- 允许执行的高风险场景、变更窗口、回滚负责人和云资源复核人。
