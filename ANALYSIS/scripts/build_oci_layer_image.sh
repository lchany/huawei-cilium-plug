...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 BASE_IMAGE NEW_IMAGE BINARY WORK_DIR" >&2
  exit 2
fi
...
tar --sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner \
  -C "$work_dir/layer" -cf "$work_dir/layer.tar" usr/bin/cilium-agent
...
jq --arg diff_id "$layer_diff_id" \
  --arg created_by "replace cilium-agent for ${new_image}" \
