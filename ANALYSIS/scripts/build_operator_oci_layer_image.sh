...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 BASE_IMAGE NEW_IMAGE OPERATOR_BINARY DESTINATION_NAME WORK_DIR" >&2
  exit 2
fi
...
[[ -x $binary ]]
[[ $destination_name =~ ^cilium-operator(-huaweicloud)?$ ]]
...
' "$work_dir/layout/index.json" >/dev/null
...
install -m 0755 "$binary" "$work_dir/layer/usr/bin/$destination_name"
tar --sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner \
  -C "$work_dir/layer" -cf "$work_dir/layer.tar" "usr/bin/$destination_name"
...
jq --arg diff_id "$layer_diff_id" --arg created_by "replace $destination_name for $new_image" \
  '.rootfs.diff_ids += [$diff_id] | .history += [{"created_by":$created_by}]' \
...
jq --arg config_digest "$new_config_digest" --argjson config_size "$new_config_size" \
  --arg layer_digest "$layer_digest" --argjson layer_size "$layer_size" \
  '.config.digest=$config_digest | .config.size=$config_size |
...
jq --arg manifest_digest "$new_manifest_digest" --argjson manifest_size "$new_manifest_size" --arg image "$new_image" \
  '.manifests[0].digest=$manifest_digest | .manifests[0].size=$manifest_size |
...
tar --sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner \
  -C "$work_dir/layout" -cf "$work_dir/image.tar" .
ctr -n k8s.io images import "$work_dir/image.tar" >/dev/null
...
echo "image_tar_sha256=$(sha256sum "$work_dir/image.tar" | awk '{print $1}')"
echo "manifest_digest=$new_manifest_digest"
