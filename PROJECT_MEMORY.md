...
- [2026-07-14] Patch archive and baseline
  Value: `/home/l30002999/huawei-cilium-v1.19.1` targets upstream Cilium v1.12.19 commit `a1d7fbd43b563c809330b1c3e28165a3e7ff43aa`. Per the corrected delivery requirement, the earliest 14 functional patches remain separate and later validation bug fixes/tests are consolidated in `0015-fix-huaweicloud-consolidate-validation-bug-fixes.patch`; `series` and the directory both contain exactly 15 patches. Audit99 clean replay commit `4f14c77b9fa0716485c1d8cdf8e5e71945888055`, patch-generation commit `eee1675ac6b7a2a29698eeb2b99498f2c4699ff2`, and authoritative source `2451099d91eeea13033e38f61c4ec8b5eb704783` share tree `67a1ccc15965f77070359b475db763f767822cca`; patch 0015 SHA256 is `375c7bec721afa11909d988eed6e99e41a3dcb9163667563c135e98b8612b052`. The latest patch/evidence changes remain local until the user explicitly requests another push.
...
- Last verified: The ledger contains 400 Pass, 41 Pending, 10 user-approved Skip, and 0 recorded Fail. Audit90 Operator remains live 1/1. Audit95 fixed the unsupported HuaweiCloud IPv6 boundary, rolled deterministic Agent+BPF images to all five nodes, and passed complete customer regression. Audit96 proved isolation under a 1000-Event backlog. Audit97 added real kernel BPF coverage for metadata-only IPv4 and VLAN/MAC mismatches. Audit98 executed a real 17-byte skb with a VLAN header truncated by one byte. Audit99 proved an exact VLAN+MAC map entry cannot redirect metadata-VLAN traffic received on a non-trunk ifindex; focused 20-repeat, full nine-object, clean replay, and post-test mesh passed. Runtime product/BPF code was unchanged by audits97-99, so audit95 Agents and audit90 Operator remain authoritative; audit64 Agent is retained as rollback. The user approved skipping STAB-01 through STAB-04 and BRACE-12 rather than waiting for 24/72-hour or equivalent long-idle stability runs. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
