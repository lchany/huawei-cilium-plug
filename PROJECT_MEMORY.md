...
- Last verified: The ledger contains 392 Pass, 54 Pending, 5 approved Skip, and 0 recorded Fail. Audit90 added bounded Create HTTP-timeout coverage and fixed fail-fast validation for unsafe endpoint/region/project configuration; the rebuilt audit90 Operator remains live 1/1 Ready/restart0. Audit91 proved external Secret identity/data/ownership remain unchanged across real isolated Helm lifecycle. Audit92 proved request-ID safety and real invalid-SG fail-closed behavior after safely freeing one unused quota slot; recovery restored exact cloud/CiliumNode state, pool40, used-outside0, error0, and mesh56/56. Audit93 used independent leader namespaces to prove both cloud API network blackhole and DNS NXDOMAIN make isolated candidates fail with attributable errors while the production Operator remains untouched; all temporary config and namespaces were removed and mesh stayed56/56. Audit90 Operator and audit64 Agents are authoritative. Metadata-only IPv4 and physically truncated skb remain Pending rather than overclaimed. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
