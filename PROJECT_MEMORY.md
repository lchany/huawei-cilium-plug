...
- Last verified: The ledger contains 386 Pass, 60 Pending, 5 approved Skip, and 0 recorded Fail. Audit90 added bounded Create HTTP-timeout coverage and fixed fail-fast validation for unsafe endpoint/region/project configuration. Focused100/race20, API20/race20, HuaweiCloud10/race10, and vet passed. The rebuilt audit90 Operator is live 1/1 Ready/restart0; pool40, used-outside0, operator errors0, mesh56/56, customer HTTP21x100, TCP4x5, and source-IP19/19 all passed. Isolated endpoint/region live negatives failed exactly as expected without replacing production and left no residue. Audit90 Operator and audit64 Agents are authoritative. Metadata-only IPv4 and physically truncated skb remain Pending rather than overclaimed. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
