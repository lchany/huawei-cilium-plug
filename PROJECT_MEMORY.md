...
- Last verified: The ledger contains 382 Pass, 64 Pending, 5 approved Skip, and 0 recorded Fail. Audit86 passed the 30-minute cleanup recheck. Audit87 added exact empty/special subnet-tag coverage and regenerated an exact 15-patch replay. Audit88 waited for real ConfigMap projection, switched one live Agent's active Pod rules from priority111 to explicit compatibility priority110 while retaining per-VLAN tables/pool/mesh, then restored priority111 and a fresh restart-zero Agent. Routing boundary suites passed 100/race20 plus privileged20/race10 and vet, covering VLAN 1/244/4094 and invalid 0/4095/parse/config inputs. The first too-early projection attempt is excluded. Audit81 Operator and audit64 Agents remain authoritative. Metadata-only IPv4 and physically truncated skb remain Pending rather than overclaimed. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
