...
- Last verified: The ledger contains 327 Pass, 120 Pending, 4 approved Skip, and 0 recorded Fail. Independent review rejected audit62's narrow route-recovery conclusion after finding empty per-VLAN tables and a missing permanent neighbor despite successful main-table traffic. Audit64 adds five-second ordinary-endpoint plus `cilium_host` router reconciliation through a non-mutating `RoutingInfo.Reconcile` path that never changes link MTU or administrative state. Live deletion of both route/rule sets and the neighbor recovered in 3 seconds with unchanged BPF maps; three real down-five-seconds/up rounds each produced exactly 125/150 followed by 20/20 while all five node0002 policy tables and the neighbor recovered without Agent restart. Clean replay/tree/patch identities are `c5b51a316fc945163b5f9b65c8634f0e14c847ad`, `99bf4f10a82852390846d1b0cc74d0f45527b334`, and patch SHA256 `38338f0326ce862c12b8896591deb6e859ba85c2d2c95cdade19f161338b9702`. Audit64 Agent SHA256 is `a7b00205e414e0c767ed8ee9ac2df0ff398fb3df4ea147c28b877a99051a446a`; manifest is `sha256:86c241725764df7a68de4df3e5919e021fc4f64b70b6147fdfa66ae12560366c`. Five-node rollout, ten recreate rounds at 56/56, HTTP 21/21 at 100/100, TCP 4/4 at 5/5, source-IP 19/19, exact map ownership, complete routes/neighbors, severe-log scan, and post-cleanup mesh all passed. Audit65 proved foreign priority-112 policy preservation and scoped legacy priority-110 cleanup. Audit66 then completed a real audit64-to-audit60-to-audit64 single-node rollback with mixed-version mesh checks, and audit67 proved explicit ErrImagePull/ImagePullBackOff isolation plus automatic recovery: the other four Agents and customer pod2-to-pod3 remained healthy, then all five returned to one audit64 imageID with complete routes and mesh 56/56. Audit60 remains the rollback image.
...
- Immediate live step: preserve the verified audit64 runtime plus audit60 rollback and execute the next isolated Pending P0/P1 recovery or fault-injection row; serialize all destructive/customer suites with the shared lock.
