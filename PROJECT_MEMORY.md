...
- Last verified: The ledger contains 353 Pass, 93 Pending, 5 approved Skip, and 0 recorded Fail. Audit80 proved a same-namespace custom Secret name rolls successfully, then proved a Secret present only in `default` produces a kube-system candidate CreateContainerConfigError while the old Operator, mesh 56/56, and exact pool mapping remain healthy. Both refs, strategy 1/1, and audit76 were restored; both temporary Secrets were removed; 40 pool entries and zero auth/severe logs were verified. Audit79 through audit76 evidence remains valid. Generic Pod readiness remained true during invalid credentials and must not be used as cloud-auth proof. Five nodes, five exact audit64 Agents, Operator audit76, routes and neighbors are healthy. Literal 40-Pod cases remain Pending rather than being silently folded into the approved min-allocate environment Skip. Audit60 remains the rollback Agent image and audit20 remains the rollback Operator image.
