...
- Last verified: The ledger contains 345 Pass, 101 Pending, 5 approved Skip, and 0 recorded Fail. Audit76 fixed API-08 so an already-deleted SubENI/404 is treated as convergence, clears stale local manager/node entries, and continues the release batch while preserving all other failures and cancellation. Targeted normal/race tests, full ENI tests, vet, clean replay equality, Operator rebuild/deployment, matrix 56/56, customer HTTP 21/21×100, TCP 4/4×5, and source-IP 19/19 all passed. Five nodes, five exact audit64 Agents, Operator audit76, 40 pool entries, used-subset checks, routes, neighbors, and logs are healthy. Audit75 and audit74 release lifecycle evidence remains valid; literal 40-Pod cases remain Pending rather than being silently folded into the approved min-allocate environment Skip. Audit60 remains the rollback Agent image and audit20 remains the rollback Operator image.
...
- Immediate live step: continue with bounded cloud-resource and recovery cases, prioritizing remaining release failure/retry, cloud API CRUD, and recovery paths that do not require unapproved environment changes.
