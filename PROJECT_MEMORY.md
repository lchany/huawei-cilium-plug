...
- Last verified: The ledger contains 390 Pass, 56 Pending, 5 approved Skip, and 0 recorded Fail. Audit90 added bounded Create HTTP-timeout coverage and fixed fail-fast validation for unsafe endpoint/region/project configuration; the rebuilt audit90 Operator remains live 1/1 Ready/restart0. Audit91 proved external Secret identity/data/ownership remain unchanged across real isolated Helm install, upgrade, rollback, and uninstall. Audit92 proved request IDs survive every normalized HTTP error class without credential leakage, then freed one confirmed-unused SubENI under a stopped Operator so a real invalid SG reached cloud validation instead of the 8/8 quota gate; it failed closed with request ID and no partial resource. Operator recovery restored pool40, exact target cloud/CiliumNode set, used-outside0, error0, and mesh56/56. Audit90 Operator and audit64 Agents are authoritative. Metadata-only IPv4 and physically truncated skb remain Pending rather than overclaimed. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
