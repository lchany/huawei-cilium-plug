...
- Last verified: The ledger contains 384 Pass, 62 Pending, 5 approved Skip, and 0 recorded Fail. Audit86 passed the 30-minute cleanup recheck; audit87 added exact empty/special subnet-tag coverage and regenerated an exact 15-patch replay; audit88 verified explicit live routing compatibility plus VLAN/table boundaries. Audit89 ran 20 concurrent rounds from customer pod2: direct remote Pod, ClusterIP, and remote-node NodePort all succeeded while public `1.1.1.1` consistently failed under `enable-ipv4-masquerade=false`, proving multi-target concurrency and public-failure isolation. Final Agents were 5/5 restart-zero with pool40 and used-outside0. Audit81 Operator and audit64 Agents remain authoritative. Metadata-only IPv4 and physically truncated skb remain Pending rather than overclaimed. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
