...
- [2026-07-14] Patch archive and baseline
  Value: `/home/l30002999/huawei-cilium-v1.19.1` targets upstream Cilium v1.12.19 commit `a1d7fbd43b563c809330b1c3e28165a3e7ff43aa`. Per the corrected delivery requirement, the earliest 14 functional patches remain separate and later validation bug fixes are consolidated in `0015-fix-huaweicloud-consolidate-validation-bug-fixes.patch`; `series` and the directory both contain exactly 15 patches. Audit81 clean replay commit `afa2b04001771132df5cf0c3c4dae83d6e4b19b0`, patch-generation commit `a8e44ef945c9133fec50824e7383c40824b2cceb`, and authoritative source `1248ceadd69bc4b4a0307bacf5bc2c9fe22bb61b` share tree `d116bd05abedd2945e72cc10e574c0238439ddac`; patch 0015 SHA256 is `0737c4a7c741a512596f86955cfb5cefb4e80a14ef4c573f435773867f5a3c1c`. The latest patch/evidence changes remain local until the user explicitly requests another push.
...
- Last verified: The ledger contains 366 Pass, 80 Pending, 5 approved Skip, and 0 recorded Fail. Audit81 executed real single/batch SubENI Create, Show, exact List, inline tag verification, Delete/NotFound, and recovery of two manually deleted idle resources. It found and fixed the old region-incompatible Port-tag finalization by putting validated tags directly in single and batch SubENI creation requests. The final production client, normal/race/full package tests, clean 15-patch replay, audit81 Operator build/deployment, customer HTTP 21/21 at 100/100, TCP 4/4 at 5/5, source-IP 19/19, and independent five-node health/matrix 56/56 all passed. A real audit81→audit76→audit81 Operator rollback/restore then kept both windows healthy, matrix 56/56, and the exact pool mapping unchanged. Audit82 reran valid Secret rotation plus wrong-namespace recovery on audit81, restored the original reference, removed all temporary Secrets, and kept pool/mesh healthy. Exact cloud/pool identity closed orphan cleanup, and repository plus Git-history high-confidence credential scans found no secret material in archived evidence. Literal 40-Pod cases remain Pending rather than being silently folded into the approved min-allocate environment Skip. Audit60 remains the rollback Agent image; audit20 and audit76 remain retained Operator rollback references, with audit76 verified against audit81.
...
- Immediate live step: continue with bounded cloud-resource and recovery cases, prioritizing remaining cloud API CRUD, permission negatives, Secret namespace/name failures, and non-destructive recovery paths that do not require unapproved environment changes.
