...
- Last verified: The ledger contains 402 Pass, 39 Pending, 10 user-approved Skip, and 0 recorded Fail. Audit90 Operator remains live 1/1. Audit95 fixed the unsupported HuaweiCloud IPv6 boundary, rolled deterministic Agent+BPF images to all five nodes, and passed complete customer regression. Audit96 proved isolation under a 1000-Event backlog. Audit97 added real kernel BPF coverage for metadata-only IPv4 and VLAN/MAC mismatches. Audit98 executed a real 17-byte skb with a VLAN header truncated by one byte. Audit99 proved non-trunk VLAN bypass with an exact map entry. Audit100 deterministically injected a VLAN-pop failure and proved `DROP_HWC_VLAN_POP_FAIL`. Audit101 passed a five-state live GRO/GSO/TSO/scatter-gather/TX-checksum matrix with per-state mesh, ICMP, and bulk TCP, then restored and reasserted the original host features. Runtime product/BPF code was unchanged by audits97-101, so audit95 Agents and audit90 Operator remain authoritative; audit64 Agent is retained as rollback. The user approved skipping STAB-01 through STAB-04 and BRACE-12 rather than waiting for 24/72-hour or equivalent long-idle stability runs. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
