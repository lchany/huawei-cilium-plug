...
- Last verified: The ledger contains 335 Pass, 111 Pending, 5 approved Skip, and 0 recorded Fail. Audit71 completed the retained audit60 old-candidate comparison: three exact 300-second streams per topology and three TCP/UDP 10000-exchange runs per topology. Relative to audit60, audit64 same-node throughput was effectively unchanged (+0.001%), cross-node throughput improved 2.613%, same-node TCP P99 improved 15.081%, and the remaining P99 changes were regressions of 1.072–5.426%, all with zero loss and within the documented conservative 10% technical guard because the customer supplied no numeric gate. All five Agents were sequentially restored to audit64; each intermediate mesh and final mesh passed 56/56. Final audit proved five Ready Nodes, exact audit64 hashes and alias manifests, zero restarts, status OK, complete priority-111 two-route tables, permanent neighbors, zero severe Agent errors, and Operator 1/1. Audit64 source/replay/tree/patch identities remain unchanged, and audit60 remains the rollback image.
...
- Immediate live step: execute PERF-04 TCP/UDP PPS with packet/drop/softirq/BPF counter snapshots, then independently audit and retain a final mesh/identity/route/health check.
