...
- [2026-07-14] Patch archive and baseline
  Value: `/home/l30002999/huawei-cilium-v1.19.1` targets upstream Cilium v1.12.19 commit `a1d7fbd43b563c809330b1c3e28165a3e7ff43aa`. Per the corrected delivery requirement, the earliest 14 functional patches remain separate and later validation bug fixes/tests are consolidated in `0015-fix-huaweicloud-consolidate-validation-bug-fixes.patch`; `series` and the directory both contain exactly 15 patches. Audit102 clean replay commit `636d531ca32fb1b631762fe942b3c2bd0007173d`, patch-generation commit `c032782492cdb30e17106077c90fcf41750b7b76`, and authoritative source `8d1ac5a642408c95fcfa822e7438b44f22ff8558` share tree `b16f850a6a8402d781897484802af06726326137`; patch 0015 SHA256 is `92f78ed9db78c00df9ae6c8c3956c6989f2f23d09f18ef9ff1d114cd9410805c`. The latest patch/evidence changes remain local until the user explicitly requests another push.
...
- Last verified: The ledger contains 403 Pass, 38 Pending, 10 user-approved Skip, and 0 recorded Fail. Audit90 Operator remains live 1/1. Audit95 fixed the unsupported HuaweiCloud IPv6 boundary, rolled deterministic Agent+BPF images to all five nodes, and passed complete customer regression. Audit96 proved isolation under a 1000-Event backlog. Audits97-100 closed the remaining VLAN parser/non-trunk/pop-failure kernel boundaries. Audit101 passed a five-state live GRO/GSO/TSO/scatter-gather/TX-checksum matrix with per-state mesh, ICMP, and bulk TCP, then restored the original host features. Audit102 filled both exact HuaweiCloud production BPF map schemas to all 65536 slots, proved overflow rejection and slot reuse, repeated on clean patch replay, and passed post-test mesh. Runtime product/BPF code was unchanged by audits97-102, so audit95 Agents and audit90 Operator remain authoritative; audit64 Agent is retained as rollback. The user approved skipping STAB-01 through STAB-04 and BRACE-12 rather than waiting for 24/72-hour or equivalent long-idle stability runs. Literal 40-Pod cases remain Pending rather than being folded into the approved min-allocate environment Skip.
