...
- Last verified: The ledger contains 349 Pass, 97 Pending, 5 approved Skip, and 0 recorded Fail. Audit78 injected a nonexistent Operator image with maxUnavailable temporarily zero: the bad candidate reached ImagePullBackOff while one old audit76 Operator stayed Ready, mesh passed 56/56, and the exact pool/resource map was unchanged; continued recovery removed the bad candidate/reference, restored audit76 and strategy 1/1, and repeated the same checks. Audit77's audit76→audit20→audit76 rollback and audit76's API-08/full acceptance evidence remain valid. Five nodes, five exact audit64 Agents, Operator audit76, 40 pool entries, routes, neighbors, and logs are healthy. Literal 40-Pod cases remain Pending rather than being silently folded into the approved min-allocate environment Skip. Audit60 remains the rollback Agent image and audit20 remains the rollback Operator image.
...
- Immediate live step: continue with bounded cloud-resource and recovery cases, prioritizing remaining cloud API CRUD, credential/permission negatives, and non-destructive recovery paths that do not require unapproved environment changes.
